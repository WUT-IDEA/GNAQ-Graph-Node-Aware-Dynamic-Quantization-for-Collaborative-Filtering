...
def save_model(recsys: FM, save_path: str):
    ...
    # place model to cpu
    recsys.cpu()
    binary_emb = recsys.embedding.binary_emb
    binary_emb.centroid_assignment = binary_emb.centroid_assignment.cpu()
    binary_emb.centroid_embs = binary_emb.centroid_embs.cpu()
    ...
    binary_emb.centroid_assignment = binary_emb.centroid_assignment.to(recsys.opt["device_id"])
...
def load_model(opt: dict, load_path: str) -> FM:
    ...
    recsys.embedding.binary_emb.opt = opt
    # load to correct device
    recsys = recsys.to(opt["device_id"])
    binary_emb = recsys.embedding.binary_emb
    binary_emb.centroid_assignment = binary_emb.centroid_assignment.to(opt["device_id"])
