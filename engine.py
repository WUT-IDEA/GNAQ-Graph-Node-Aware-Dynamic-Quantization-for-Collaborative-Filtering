...
def run():
    ...
    for model_name in ["best_checkpoint_with_assignment_update"]:
        ...
        recsys = load_model(opt, checkpoint_save_path).to(opt["device_id"])
        # save best centroid embs
        centroid_save_path = os.path.join(recsys.embedding.binary_emb.centroid_save_path,
                                          f"{model_name}.npz")
        sp.save_npz(centroid_save_path,
                    sp.csr_matrix(recsys.embedding.binary_emb.centroid_embs.data.detach().cpu().numpy()))
        evaluated_start = timer()
...
if __name__ == "__main__":
    ...
    try:
        ...
        time_end = timer()
        print_text(performance_fp, f"Program ends. Time elapsed = {timedelta(seconds=time_end - time_start)}.")
        writer.close()
